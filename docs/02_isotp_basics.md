...
참고: [Step 1. CAN Frame Parser](docs/01_can_frame_parser.md)
...
python -m pytest tests/test_isotp_frame.py tests/test_isotp_payload_reassembler.py tests/test_send_isotp_scenario.py
```
...
다음 단계에서는 현재 데모 수준의 ISO-TP 처리를 transport layer로 끌어올려야 합니다.
...
- `tests/test_send_isotp_scenario.py`
