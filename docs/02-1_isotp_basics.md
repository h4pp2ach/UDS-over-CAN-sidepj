...
- frame type별 필드 추출
- frame 자체가 성립하지 않는 경우 `IsoTpFrameParseError` 발생
...
- 잘못된 frame 순서나 오염된 조립 상태를 `IsoTpReassemblyError`로 처리하고 reset
...
- 분할할 수 없는 payload를 `IsoTpSegmentationError`로 구분


세 오류는 모두 `IsoTpValidationError(ValueError)`를 상속합니다. 따라서 기존 `ValueError` 처리와 호환하면서 parser, reassembler, segmenter 중 어느 단계에서 실패했는지 구분할 수 있습니다. 함수 계약에 맞지 않는 Python 타입은 protocol 데이터 오류와 섞지 않고 `TypeError`로 처리합니다.
...
<br />
선택한 방식: 조립 중 `SingleFrame`이나 새 `FirstFrame`이 들어오면 현재 구현은 상태를 reset하고 `IsoTpReassemblyError(ValueError)`를 던집니다.
...
<details close>
<summary><strong>6. ISO-TP 예외 계층을 공통 모듈로 분리한 이유</strong></summary>
<br />

처음에는 parser, reassembler, segmenter가 각각 `ValueError` 또는 `TypeError`를 직접 발생시키고, transport가 그중 일부 `ValueError`를 `IsoTpProtocolError`로 변환했습니다. 구현은 짧지만 컴포넌트가 늘어나면서 다음 문제가 생깁니다.

- 호출자는 같은 `ValueError`만 보고 어느 단계에서 실패했는지 구분하기 어려움
- transport가 `ValueError`를 넓게 잡으면 예상한 protocol 실패뿐 아니라 내부 구현 실수까지 protocol 오류로 오분류할 수 있음
- 반대로 변환하지 않은 `TypeError`나 `AttributeError`가 입력 모양에 따라 그대로 노출되면 공개 API가 불안정해짐
- 하위 예외를 새 예외로 바꾸는 과정에서 원인을 보존하지 않으면 최초 실패 위치와 traceback을 추적하기 어려움

선택한 방식은 예외 정의를 `isotp_errors.py`에 모으고, core validation과 transport 공개 API를 두 개의 경계로 구분하는 것입니다.

```text
IsoTpValidationError(ValueError)
...
여기서 중요한 것은 다음 변환 원칙입니다.

```text
잘못된 Python API 타입
    └── TypeError 유지

core에서 의미가 확인된 ISO-TP validation 실패
    └── 발생 컴포넌트가 구체적인 domain error를 생성
            └── transport 경계를 통과할 때 알려진 domain error만 변환
                    └── raise ... from exc 로 최초 원인 보존
```

이 구조를 선택한 이유:

- parser, reassembler, segmenter가 같은 validation 예외 정책 공유
- broad `except ValueError` 대신 구체적인 domain error만 잡으므로 예상하지 못한 구현 오류를 숨기지 않음
- 테스트가 메시지 문자열뿐 아니라 실패 단계, 공개 예외, 원인 예외의 연결까지 검증할 수 있음

검토한 다른 방식:

- 모든 실패를 `ValueError` 하나로 처리
  → 구현은 가장 짧지만 호출자가 실패 원인과 대응 정책을 구분하기 어려움
- 모든 ISO-TP 실패를 하나의 `IsoTpError`로 평탄화
  → catch는 단순하지만 core 입력 검증과 transport 실행 실패의 경계가 다시 섞임
- 하위 예외를 transport에서 변환하지 않고 그대로 노출
  → 최초 예외는 보존되지만 transport API가 parser와 reassembler의 내부 구현에 결합됨
- status code 또는 result object 반환
  → 실패를 명시적으로 표현할 수 있지만 모든 호출부가 반환값 검사를 반복해야 하고 현재 Python API에는 과함

현재 구조는 core와 transport를 분리하면서도 기존 `ValueError` 호환성, 공개 API의 안정성, 원인 추적 가능성을 함께 유지할 수 있음.
...
- `src/isotp_errors.py`
  - frame parsing, 재조립, segmentation 단계의 구체적인 validation 예외를 정의합니다.
